// Simulação de conexão WebRTC no mesmo computador
(function() {
...
(function() {
  ...
  var remoteDescription = null;// RTCPeerConnection for the "remote"
  ...
  var customEvent = {
    type: 'custom-event',
    data: 'Dados do evento personalizado',
  };
  ...
  function connectPeers() {
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (response.type === 'ice-candidate') {
        ...
        .then(offer => localConnection.setLocalDescription(offer))
        .then(() => {
          socket.send(JSON.stringify({type: 'offer', data: localConnection.localDescription}));
          console.log('descrição enviada ao servidor!');
        })
        .catch(handleCreateDescriptionError);
      }
      if (response.type === 'offer') {
        remoteDescription = response.data;
